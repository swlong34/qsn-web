<template>
  <div class="qs-layout">
    <div class="sr-show" @mouseenter="changePop" @mouseleave="popup = false">
      ...
      <div>
        <div class="sr-title"><span>{{qsIndex}}.&nbsp;{{nameForm.title}}</span></div>
        <el-input
  ...
  export default {
    ...
    mounted() {
      let name = {
        ...
        qs_title: this.nameForm.title,
        qs_type: this.QS_TYPE.SHORTCUT_NAME.value,
        qs_option: this.nameForm.option
      }
      this.$emit('add2Paper', name)
    },
    methods: {
      changePop() {
