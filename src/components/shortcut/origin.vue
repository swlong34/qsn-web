<template>
  <div class="qs-layout">
    <div class="sr-show" @mouseenter="changePop" @mouseleave="popup = false">
      ...
      <div>
        <div class="sr-title"><span>{{qsIndex}}.&nbsp;{{OriginForm.title}}</span></div>
        <el-input
  ...
  export default {
    ...
    mounted() {
      let origin = {
        ...
        qs_title: this.OriginForm.title,
        qs_type: this.QS_TYPE.SHORTCUT_ORIGIN.value,
        qs_option: this.OriginForm.option
      }
      this.$emit('add2Paper', origin)
    },
    methods: {
      changePop() {
