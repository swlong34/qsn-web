<template>
  <div class="qs-layout">
    <div class="sr-show" @mouseenter="changePop" @mouseleave="popup = false">
      ...
      <div>
        <div class="sr-title"><span>{{qsIndex}}.&nbsp;{{BirthdayForm.title}}</span></div>
        <el-date-picker
          v-model="ans"
          type="date">
        </el-date-picker>
  ...
  export default {
    ...
    mounted() {
      let birthday = {
        ...
        qs_title: this.BirthdayForm.title,
        qs_type: this.QS_TYPE.SHORTCUT_BIRTHDAY.value,
        qs_option: this.BirthdayForm.option
      }
      this.$emit('add2Paper', birthday)
    },
    methods: {
      changePop() {
