<template>
  <div class="qs-layout">
    <el-form v-if="show" :model="fillMultiForm" ref="fillMultiForm" label-width="100px" class="demo-dynamic">
      <el-form-item prop="title" label="标题[多行]" :rules="{
      ...
      <el-form-item>
        <el-button size="small" type="primary" @click="finishForm('fillMultiForm')">完成</el-button>
        <el-button size="small" type="danger" @click="deleteForm('fillMultiForm')">删除</el-button>
      </el-form-item>
    </el-form>
    ...
    <div v-if="!show" class="sr-show" @mouseenter="changePop" @mouseleave="popup = false">
      ...
      <div class="sr-left">
        <div class="sr-title"><span>{{qsIndex}}.&nbsp;{{fillMultiForm.title}}</span></div>
        <el-input
          type="textarea"
    ...
    created() {
      ...
      if(this.qsModel === 'true') {
        ...
        this.$axios({
          ...
          this.fillMultiForm.qs_id = qs_id
          this.fillMultiForm.title = qs_title
          this.fillMultiForm.options = qs_option
        })
      }

      // 若为编辑状态，则需要获取题目初始信息
      if (this.qsEdit === 'true') {
        console.log("编辑多行文本，题目数据渲染")
        this.show = false
        this.$axios({
          method: 'get',
